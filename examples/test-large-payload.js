...
var longString = "/";
for (var i=0; i<80000; i++) longString += "a";
...
client.subscribe('sidekick.requests', function (data) { 
  console.log("Sidekick Client: asserting...");
  assert.equal(data.path, longString);
});
...
setInterval(function () { 
  ...
  http.get({ host: 'localhost', port: ORIG_PORT, path: longString }, function (res) {});
}, 1000);
